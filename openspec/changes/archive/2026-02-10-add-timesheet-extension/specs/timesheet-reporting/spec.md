...
The extension SHALL provide a comprehensive timesheet view showing all time entries for the current user across a specified date range.

#### Scenario: Access user timesheet from work item tab
- **WHEN** user is viewing the Time Sheet tab on a work item
- **THEN** a "My Timesheet" button or link SHALL be available
- **WHEN** user clicks it
- **THEN** a timesheet view SHALL open showing all their time entries
...
- **WHEN** viewing timesheet or team reports
- **THEN** a pie or bar chart showing hours by activity type SHOULD be displayed
